...
public class ExcelXlsxReaderWithDefaultHandler extends DefaultHandler {
	 ...
	 * @return
     */
    public List<List<String>> process(InputStream inputStream) throws Exception {
        ...
        while (sheets.hasNext()) {
			...
			// 解析excel的每条记录
			// 在这个过程中startElement()、characters()、endElement()这三个函数会依次执行
    ...
    public void startElement(String uri, String localName, String name, Attributes attributes) {
        ...
        if ("c".equals(name)) {
            ...
            if (!startElementFlag && !flag) {
            	// 上一个单元格为文本空单元格，执行下面的，使ref=preRef；
				// flag为true表明该单元格之前有数据值，即该单元格不是首部空单元格，则跳过
    ...
    public void endElement(String uri, String localName, String name) {
        ...
        if (isTElement) {// 这个程序没经过
            ...
            if ("row".equals(name)) {
                ...
                if (flag && curRow != 1 && CollectionUtils.isNotEmpty(cellList)) {
                    // 该行不为空行且该行不是第一行，则发送（第一行为列名，不需要）
                    List<String> cellValues = Lists.newArrayList();
                    Collections.addAll(cellValues, new String[cellList.size()]);
                    Collections.copy(cellValues, cellList);
                    dataList.add(cellValues);
                    totalRows++;
                }
                cellList.clear();
                curRow++;
