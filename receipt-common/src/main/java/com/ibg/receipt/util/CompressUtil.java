...
public class CompressUtil {
    ...
    public static File[] unzip(String zip, String dest, String passwd)
        ...
        return CompressUtil.unzip(zipFile, dest, passwd);
    }
    ...
    public static File[] unzip(String zip, String passwd) throws ZipException {
        ...
        return CompressUtil.unzip(zipFile, parentDir.getAbsolutePath(), passwd);
    }
    ...
    public static File[] unzip(File zipFile, String dest, String passwd)
        ...
        for (FileHeader fileHeader : headerList) {
            if (!fileHeader.isDirectory()) {
                extractedFileList
                    .add(new File(destDir, fileHeader.getFileName()));
            }
        }
    ...
    private static String buildDestinationZipFilePath(File srcFile,
            String destParam) {
        if (org.apache.commons.lang3.StringUtils.isEmpty(destParam)) {
            if (srcFile.isDirectory()) {
            ...
            CompressUtil.createDestDirectoryIfNecessary(destParam); // 在指定路径不存在的情况下将其创建出来
    ...
    private static void createDestDirectoryIfNecessary(String destParam) {
        ...
        if (destParam.endsWith(File.separator)) {
            ...
            destDir = new File(
                destParam.substring(0, destParam.lastIndexOf(File.separator)));
        }
        if (!destDir.exists()) {
            destDir.mkdirs();
        }
